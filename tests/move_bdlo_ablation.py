...
class DEFTOptimizationProblem:
    ...
    def intermediate(self, alg_mod, iter_count, obj_value, inf_pr, inf_du, mu,
        ...
        if iter_count >= 10 and not self.converged:
            print(f"  *** FAILED: not converged after {iter_count} IPOPT iterations ***")
            return False  # Stop optimization
    ...
    def gradient(self, x):
        ...
        # Zero out v0/v1 gradient (first 3 DOFs) — only optimize v11/v12
        grad_np[:3] = 0.0
        return grad_np
...
def trajectory_optimization_ipopt(deft_sim, b_DLOs_vertices_traj, previous_b_DLOs_vertices_traj,
    ...
    # Pin v0/v1 (first 3 DOFs) to zero, free v11/v12 (last 3 DOFs)
    lb = [0.0, 0.0, 0.0, -1e20, -1e20, -1e20]
    ub = [0.0, 0.0, 0.0,  1e20,  1e20,  1e20]
...
def run_ablation(config_name, checkpoint_path, use_orientation, use_attachment,
                 b_DLOs_vertices_traj, previous_b_DLOs_vertices_traj, target_b_DLOs_vertices_traj):
    ...
    print(f"  orientation_constraints={use_orientation}, attachment_constraints={use_attachment}")
    print(f"{'='*60}")

    torch.set_default_dtype(torch.float64)
    deft_sim, _, parent_theta_clamp, child1_theta_clamp, child2_theta_clamp = setup_deft_sim(
        load_checkpoint=checkpoint_path,
        use_orientation_constraints=use_orientation,
        use_attachment_constraints=use_attachment
    )
    ...
    print(f"Gradient (v11/v12 row): {grad[1].numpy()}")
    ...
    # IPOPT optimization
    if not IPOPT_AVAILABLE:
        print("IPOPT not available, skipping optimization.")
        return None

    optimized_displacements, info, optimized_traj, elapsed_time, converged = trajectory_optimization_ipopt(
        deft_sim, b_DLOs_vertices_traj, previous_b_DLOs_vertices_traj,
        target_b_DLOs_vertices_traj, parent_theta_clamp, child1_theta_clamp, child2_theta_clamp
    )

    print(f"\n*** {config_name} OPTIMIZATION TIME: {elapsed_time:.3f} seconds ***")
    print(f"Final objective: {info['obj_val']:.6f}")
    if converged:
        print(f"Result: SUCCESS — all endpoints within 0.025")
    else:
        print(f"Result: FAILED — endpoints NOT within 0.025")

    # Save animation
    anim = animate_prediction(optimized_traj, target_b_DLOs_vertices_traj, title_prefix=f"{config_name} Optimized")
    filename = f"optimized_trajectory_{config_name}.mp4"
    anim.save(filename, writer='ffmpeg', fps=20)
    print(f"Animation saved to: {filename}")

    return optimized_traj, elapsed_time, info['obj_val'], converged
...
if __name__ == "__main__":
    ...
    ALL_ABLATIONS = {
        "DEFT": {
            "checkpoint": "../save_model/BDLO1/DEFT_ends_1_pretrained_full_model.pth",
            "use_orientation": True,
            "use_attachment": True,
        },
        "DEFORM_PLUS": {
            "checkpoint": "../save_model/BDLO1/DEFT_ends_1_DEFORM_plus.pth",
            "use_orientation": False,
            "use_attachment": False,
        },
        "DEFT_ABLATION": {
            "checkpoint": "../save_model/BDLO1/DEFT_ends_1_DEFT_ablation.pth",
            "use_orientation": False,
            "use_attachment": True,
        },
    }
    ...
    if args.config == "all":
        ...
        ABLATIONS = {args.config: ALL_ABLATIONS[args.config]}

    # --- Load data (shared across all ablations) ---
    torch.set_default_dtype(torch.float64)
    eval_dataset = Eval_DEFTData(
    ...
    for i, data in enumerate(eval_loader):
        ...
        raise ValueError(f"Sample index {args.sample_idx} out of range")
    ...
    print(f"Sample {args.sample_idx}, initial_frame={initial_frame}, target_frame={target_frame}")
    ...
    for name, cfg in ABLATIONS.items():
        result = run_ablation(
            name, cfg["checkpoint"], cfg["use_orientation"], cfg["use_attachment"],
            b_DLOs_vertices_traj, previous_b_DLOs_vertices_traj, target_b_DLOs_vertices_traj
        )
    ...
    if results:
        ...
        print(f"{'='*60}")
        for name, (traj, elapsed, final_cost, converged) in results.items():
            status = "SUCCESS" if converged else "FAILED"
            print(f"  {name:20s}: {status}, cost={final_cost:.6f}, time={elapsed:.3f}s")
