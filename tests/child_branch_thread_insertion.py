...
# Valid (kinova, franka, target) dataset combinations for mocap + hole files
VALID_KINOVA_FRANKA_TARGET = frozenset({
    (1, 1, 1), (1, 1, 2),
    (2, 1, 1), (2, 1, 2),
    (3, 1, 1), (3, 1, 2),
    (4, 1, 1), (4, 1, 2),
...
def warn_invalid_kinova_franka_target(kinova_id, franka_id, target_id):
    """
    If (kinova, franka, target) is not a supported combination, emit a UserWarning and list all valid combinations.
    """
    key = (int(kinova_id), int(franka_id), int(target_id))
    if key in VALID_KINOVA_FRANKA_TARGET:
        return
    lines = [
        f"The selected combination does not exist: kinova {kinova_id} franka {franka_id} target {target_id}.",
        "Please use one of the following valid combinations:",
    ]
    for k, f, t in sorted(VALID_KINOVA_FRANKA_TARGET):
        lines.append(f"  kinova {k} franka {f} target {t}")
...
def load_mocap_config(kinova_id, franka_id):
    ...
    pts[:, 1] = -pts[:, 1]   # flip y to match the DEFT BDLO5 frame
    return pts


def load_hole(target_id, franka_id):
    """Load hole points (4 vertices) from target file."""
    fpath = os.path.join(DATASET_DIR, f'child_insertion_target_{target_id}_transformed_franka{franka_id}.pkl')
    if not os.path.isfile(fpath):
        pattern = os.path.join(DATASET_DIR, 'child_insertion_target_*_transformed_franka*.pkl')
        avail = sorted(os.path.basename(p) for p in glob.glob(pattern))
        msg = (
            f"Hole file not found:\n  {fpath}\n"
            f"Only target hole IDs 1 and 2 are shipped in this dataset; "
            f"each (target, franka) pair must match an existing file.\n"
        )
        if avail:
            msg += "Available files:\n  " + "\n  ".join(avail)
        else:
            msg += f"(No child_insertion_target_*_transformed_franka*.pkl under {DATASET_DIR})"
...
def compute_hole_center_and_normal(hole_pts):
    """Compute hole center and normal (facing TOWARD the negative direction in Z-axis).
    ...
    # Orient normal TOWARD the negative direction in Z-axis
    if normal[2] > 0 :
        normal = -normal
...
def setup_deft_sim(parent_clamped_selection, load_checkpoint=None):
    ...
    _c2_momentum = torch.zeros(batch, 2, 3, 3, dtype=torch.float64)
    _c2_momentum[:, 1] = torch.eye(3, dtype=torch.float64)
    deft_sim.child2_momentum_scale_previous = _c2_momentum
    deft_sim.child2_momentum_scale_next = _c2_momentum.clone()
...
def visualize_initial_state(initial_branched, target, hole_pts, hole_center, hole_normal,
    ...
    if interactive:
        ...
        out_dir = os.path.join(os.path.dirname(__file__), 'visualization', 'child_branch_thread_insertion')
...
def run_child_branch_insertion(kinova_id, franka_id, target_id, checkpoint_path=None,
                               ...
                               show_initial_plot=True, gradient_check=False):
    ...
    try:
        hole_pts = load_hole(target_id, franka_id)                     # (4, 3)
    except:
        hole_pts = load_hole(target_id, 3)   

    # Compute hole center & normal oriented toward the child2 tip
    # In flat format, tip is vertex 17
    tip_position = flat_pts[17]
    hole_center, hole_normal = compute_hole_center_and_normal(hole_pts)
    print(f"Hole center: {hole_center}")
    print(f"Hole normal: {hole_normal} (points toward negative Z-axis)")
    ...
    # 3. Visualize initial state (optional multi-view 3D PNGs + optional interactive window)
    ...
    full_traj = torch.cat([traj1, traj2], dim=1)

    return full_traj, initial_branched, target_insert, hole_pts, flat_pts, sol2, info2
...
def visualize_result(optimized_traj, initial_branched, target_pt, hole_pts, flat_pts,
    ...
    tri = [0, 1, 2, 0]
    ax1.plot(hole_pts[tri, 0], hole_pts[tri, 1], hole_pts[tri, 2],
             '^-', color='green', linewidth=2, markersize=8, label='Hole')
    if len(hole_pts) >= 4:
        ax1.scatter(*hole_pts[3], color='lime', s=60, marker='*', edgecolors='black', zorder=5)
    ...
    ax2.plot(hole_pts[tri, 0], hole_pts[tri, 2], '^-', color='green', linewidth=2, label='Hole')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--target', type=int, default=1, choices=[1, 2],
                        help='Target hole ID (only 1 and 2 exist in dataset/BDLO_child_branch_thread_insertion)')
    ...
    parser.add_argument('--no-initial-plot', action='store_true',
                        help='Skip initial visualization entirely (no static view PNGs, no window; for batch)')
    ...
    warn_invalid_kinova_franka_target(args.kinova, args.franka, args.target)
    ...
    if args.vis_only:
        if args.no_initial_plot:
            print("Nothing to do: --vis-only with --no-initial-plot.")
            sys.exit(0)
        torch.set_default_dtype(torch.float64)

        flat_pts = load_mocap_config(args.kinova, args.franka)
        try:
            hole_pts = load_hole(args.target, args.franka)
        except:
            hole_pts = load_hole(args.target, 3)
    ...
    result = run_child_branch_insertion(
        ...
        show_initial_plot=not args.no_initial_plot,
        gradient_check=args.gradient_check,
    )
    optimized_traj, initial_branched, target_pt, hole_pts, flat_pts, solution, info = result

    vis_dir = os.path.join(os.path.dirname(__file__), 'visualization', 'child_branch_thread_insertion')
    ...
    with open(f'{out_dir_pkl}/{tag}.pkl', 'wb') as f:
        pickle.dump(optimized_traj[0, :, 0, 0, :], f)
