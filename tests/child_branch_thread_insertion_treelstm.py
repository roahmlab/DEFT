...
Uses IPOPT with a TreeLSTM BDLO5 baseline model as the forward model (drop-in
replacement for the DEFT version in child_branch_thread_insertion.py — same
IPOPT setup, same clamping, just the forward physics model swapped).
...
sys.path.append(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'baseline'))
from deft.utils.util import DEFT_initialization, construct_b_DLOs, construct_BDLOs_data, clamp_index, index_init
from baseline.tree_lstm import BDLOTreeLSTM
...
class TreeLSTMSimWrapper(nn.Module):
    """Drop-in replacement for DEFT_sim.iterative_predict using the TreeLSTM
    baseline. Same signature so the IPOPT code doesn't need to change.

    The wrapper rolls the TreeLSTM out step-by-step: at each timestep we build
    a "clamped target hint" tensor (zeros except at parent-clamped slots,
    which hold the interpolated target positions), call the model for a
    1-step prediction, hard-enforce clamps on the output, then advance.
    """

    def __init__(self, treelstm_model, n_branch, n_vert, n_parent, parent_clamped_selection, undeformed_vert):
        super().__init__()
        self.treelstm = treelstm_model
        self.n_branch = n_branch
        self.n_vert = n_vert
        # Convert any negative indices to absolute (they're used to index into
        # the fixed-size padded tensor of n_parent parent vertices).
        self.parent_clamped_indices = [
            int(i) if int(i) >= 0 else int(i) + n_parent
            for i in parent_clamped_selection.tolist()
        ]
        self.register_buffer('undeformed_vert', undeformed_vert)

    def iterative_predict(
        self, time_horizon, b_DLOs_vertices_traj, previous_b_DLOs_vertices_traj,
        clamped_positions, dt, parent_theta_clamp, child1_theta_clamp,
        child2_theta_clamp, inference_1_batch, vis_type=None, vis=False,
    ):
        # b_DLOs_vertices_traj: [batch, 1, n_branch, n_vert, 3]
        # clamped_positions:    [batch, time_horizon, n_branch, n_vert, 3]
        current = b_DLOs_vertices_traj[:, 0]
        previous = previous_b_DLOs_vertices_traj[:, 0]
        idx = self.parent_clamped_indices
        preds = []
        for t in range(time_horizon):
            hints = torch.zeros_like(current)
            hints[:, 0, idx] = clamped_positions[:, t, 0, idx]
            pred = self.treelstm(current, previous, hints)
            pred = pred.clone()
            pred[:, 0, idx] = clamped_positions[:, t, 0, idx]
            previous = current
            current = pred
            preds.append(pred)
        predicted = torch.stack(preds, dim=1)
        return predicted, None


# Default hidden_size from baseline/train_tree_lstm.py. Kept at module scope so
# it's easy to override if you train with a different width.
TREE_LSTM_HIDDEN_SIZE = 128


def setup_deft_sim(parent_clamped_selection, load_checkpoint=None):
    """Build the TreeLSTM-based sim wrapper for child-branch thread insertion.

    Name kept as `setup_deft_sim` so the rest of the file (IPOPT, viz, main)
    doesn't need to change. The returned object exposes `.iterative_predict`,
    `.undeformed_vert`, `.parameters()`, `.eval()` — same as DEFT_sim here.
    """
    torch.set_default_dtype(torch.float64)
    device = "cpu"

    # BDLO5 undeformed shape (same numbers as the DEFT version).
    ...
    # Same pipeline the DEFT version uses — keep it here so `undeformed_vert`
    # matches the mocap input frame expected by the IPOPT problem.
    ...
    _, parent_theta_clamp, child1_theta_clamp, child2_theta_clamp = clamp_index(
        batch, parent_clamped_selection, child1_clamped_selection, child2_clamped_selection,
        N_BRANCH, N_PARENT, True, False, False
    )

    treelstm_model = BDLOTreeLSTM(
        hidden_size=TREE_LSTM_HIDDEN_SIZE,
        n_parent_vertices=N_PARENT,
        cs_n_vert=(N_CHILD1, N_CHILD2),
        rigid_body_coupling_index=COUPLING_INDEX,
        input_size=9,
        bdlo5=True,
    ).double().to(device)

    deft_sim = TreeLSTMSimWrapper(
        treelstm_model=treelstm_model,
        n_branch=N_BRANCH,
        n_vert=N_PARENT,
        n_parent=N_PARENT,
        parent_clamped_selection=parent_clamped_selection,
        undeformed_vert=b_undeformed_vert.double(),
    ).to(device)

    if load_checkpoint:
        checkpoint = torch.load(load_checkpoint, map_location=device)
        # TreeLSTM checkpoints from baseline/train_tree_lstm.py are model-
        # state dicts for BDLOTreeLSTM — load directly into the inner model.
        treelstm_model.load_state_dict(checkpoint, strict=False)
        print(f"Loaded TreeLSTM checkpoint from {load_checkpoint}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint', type=str,
                        default=os.path.join(os.path.dirname(os.path.abspath(__file__)), 'TreeLSTM_ends_5_6600_1.pth'),
                        help='TreeLSTM BDLO5 checkpoint path (default = best eval RMSE checkpoint, co-located with this script)')
