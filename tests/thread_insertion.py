...
def compute_hole_center_and_normal(hole_pts, bdlo_bulk_center):
    ...
    # Orient normal TOWARD BDLO bulk
    to_bdlo = bdlo_bulk_center - center
    if np.dot(normal, to_bdlo) < 0:
        normal = -normal
...
def visualize_initial_state(initial_branched, target, hole_pts, hole_center, hole_normal, config_id, height_id, hole_side):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(os.path.dirname(__file__), 'thread_insertion_output',
                f'initial_config{config_id}_H{height_id}{hole_side}.png'), dpi=150)
...
def run_thread_insertion(config_id, height_id, hole_side, checkpoint_path=None,
                         offset_distance=0.05, align_distance=0.05):
    ...
    """
    torch.set_default_dtype(torch.float64)
    os.makedirs(os.path.join(os.path.dirname(__file__), 'thread_insertion_output'), exist_ok=True)
    ...
    # Compute hole center & normal oriented toward the TIP (vertex 0)
    ...
    sol1, info1, traj1, state_after_align, time1 = _run_single_stage(
        ...
        parent_clamped_selection, "Stage1-Align", run_grad_check=True
    )
...
if __name__ == "__main__":
    ...
    # Always save animation (like move_bdlo_ablation.py)
