...
class DEFTOptimizationProblem:
    ...
    def objective(self, x):
        ...
        # Early stopping: all endpoints within 0.025
    ...
    def gradient(self, x):
        ...
        # Zero out v11/v12 gradient (first 3 DOFs) — only optimize v0/v1
        grad_np[-3:] = 0.0
        return grad_np
...
def trajectory_optimization_ipopt(deft_sim, b_DLOs_vertices_traj, previous_b_DLOs_vertices_traj,
    ...
    # Free v0/v1 (first 3 DOFs), pin v11/v12 (last 3 DOFs) to zero
    lb = [-1e20, -1e20, -1e20, 0.0, 0.0, 0.0]
    ub = [1e20,  1e20,  1e20, 0.0, 0.0, 0.0]
...
def run_deft(i, checkpoint_path, b_DLOs_vertices_traj, previous_b_DLOs_vertices_traj, target_b_DLOs_vertices_traj,
    ...
    with open(f'{out_dir_pkl}/test_{i}.pkl', 'wb') as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--full_batch", action="store_true", default=False, help="Whether to test all samples in a run")
    parser.add_argument("--gradient_check", action="store_true", default=False, help="Run gradient check before IPOPT optimization")
    args = parser.parse_args()

    if args.checkpoint:
        checkpoint_path = args.checkpoint
    else:
        checkpoint_path = os.path.join(os.path.dirname(__file__), '..', 'save_model', 'BDLO1', 'DEFT_ends_1_pretrained_full_model.pth')
    ...
    test_dataset = Test_DEFTData(
        ...
        eval_set_number=1, total_time=500, eval_time_horizon=498, device="cpu", bdlo5=False
    )
    test_dataset = DataLoader(test_dataset, batch_size=1, shuffle=False)

    if args.full_batch:
        test_list = [index for index in range(len(test_dataset))]
    elif args.sample_idx < len(test_dataset):
        test_list = [args.sample_idx]
    else:
    ...
    for i, data in enumerate(test_dataset):
        if i in test_list:
            ...
            result = run_deft(
                i, checkpoint_path, b_DLOs_vertices_traj, previous_b_DLOs_vertices_traj, target_b_DLOs_vertices_traj,
