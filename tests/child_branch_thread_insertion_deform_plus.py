...
ABLATION: deform+
- attachment constraints:       OFF (naive connection)
...
def setup_deft_sim(parent_clamped_selection, load_checkpoint=None):
    ...
    # ABLATION (deform+): use normal mass/MOI scales (parent_mass_scale=1,
    ...
    deft_sim = DEFT_sim(
        ...
        use_orientation_constraints=False, use_attachment_constraints=False,
        bdlo5=True
    )

    # ABLATION (deform+): no c2↔c1 extreme coupling/momentum overrides; the
    # default initializer values are used.
